...
def validate_gt(m, opt, cfg, gt_val_dataset, heatmap_to_coord, batch_size=24, pred_root=False, test_vertice=False):
    ...
    for inps, labels, img_ids, bboxes in gt_val_loader:
        ...
        output = m(
            ...
            joint_root=root, depth_factor=depth_factor,flip_output=False)
        if test_vertice:
        ...
        if opt.flip_test:
            ...
            output_flip = m(inps_flip,
                            trans_inv=trans_inv, intrinsic_param=intrinsic_param, joint_root=root,
