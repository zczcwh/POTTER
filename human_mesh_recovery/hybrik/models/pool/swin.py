...
#################################################
class PatchEmbed_R(nn.Module):

    def __init__(self, stride=2,
                 in_chans=256, embed_dim=128, norm_layer=None):
        super().__init__()
        self.stride = stride
        self.proj = nn.ConvTranspose2d(in_chans, embed_dim, kernel_size=stride, stride=stride )
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()

        # self.pos = pos_embed
        # self.pos_embed = nn.Parameter(torch.randn(1, H * stride, W * stride) * .02)

    def forward(self, x):
        # bs,dim,h,w = x.shape
        # # x = x.view(bs,-1,h * self.stride, w * self.stride )
        x = self.proj(x)
        x = self.norm(x)
...
class STAGE1(nn.Module):
    ...
    def __init__(self, dim, layers, mlp_ratio,
        ...
        mlp_ratio = [2,2,2,2]
        depth0 = layers[-1]
        # mlp_ratio0 = mlp_ratio[-1]
        # dpr0 = [x.item() for x in torch.linspace(0, drop_path_rate, depth0)]

        # self.patch_emb0 = nn.Conv2d(dim[0] * 64, dim[3], 1, groups=dim[3])
        # self.Block0 = nn.Sequential(*[PoolFormerBlock(dim[0], mlp_ratio=mlp_ratio0,
        #                                              act_layer=act_layer, norm_layer=norm_layer,
        #                                              drop=drop_rate, drop_path=dpr0[i],
        ...
        self.patch_emb1 = PatchEmbed_R(stride=2,
    ...
    def forward(self, x):
        ...
        # x_out = [x3_new, x2_new, x1_new, x0_new]
        return x3_new, x3



# class PoolAttn_pose(BaseBackbone):
class PoolAttnFormer_2S(nn.Module):
    """
    ...
    def __init__(self, img_size=224, layers=None, embed_dims=None,
        ...
        # self.patch_embed_down = PatchEmbed(
        #     patch_size=in_patch_size*8, stride=in_stride*8, padding=in_pad*8,
        #     in_chans=3, embed_dim=embed_dims[-1])
        ...
        #     size_H = img_size[0] // (2 ** (i + 2))
        #     H.append(size_H)
        #     size_W = img_size[1] // (2 ** (i + 2))
        #     W.append(size_W)
        ...
        self.stage1 = STAGE1(embed_dims, layers, mlp_ratio=mlp_ratios,
        ...
        #
        # self.gelu = nn.GELU()
        # self.final_dim = embed_dims[0] + embed_dims[1]//4 + embed_dims[2]//16 + embed_dims[3]//64
        self.norm0 = norm_layer(embed_dims[0])
        self.norm3 = norm_layer(embed_dims[3])
        # self.norm0 = norm_layer(512)

        img_size = [img_size[1], img_size[0]]

        # self.final_conv = nn.Conv2d(embed_dims[0],1,kernel_size=2, stride=2)
        # self.final_dim = (img_size[0] // 8) * (img_size[1] // 8)
        #
        # self.final_layer = nn.Linear(self.final_dim,num_classes)
...
# import torch
# model_deit = SwinTransformer()
# input = torch.ones(2,3,224,224).cuda()
# model_pos = model_deit.cuda()
# pytorch_total_params = sum(p.numel() for p in model_pos.parameters()) / 1_000_000
# y= model_pos(input)
# # parameters = sum([np.prod(p.size()) for p in parameters]) / 1_000_000
# print('Total Parameters: %.3fM' % pytorch_total_params)
# # print(y.shape)
# from ptflops import get_model_complexity_info
# macs, params = get_model_complexity_info(model_pos, (3,224,224), as_strings=True,
